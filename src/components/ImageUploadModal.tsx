...
import { useUserImages, useUploadImage, useDeleteImage } from '@/hooks/useImage/useImage';
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] w-full bg-[#0A0A29] text-white overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden pr-2">
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-sm text-gray-400">Supported: JPEG, PNG, WebP (Max 10MB)</p>
            <input
              id="file-input"
              type="file"
        ...
        <div className="mt-6">
          ...
          {!isLoading && images && images.length > 0 && (
            <div className="grid grid-cols-4 gap-2 mt-2">
              {images.map((img) => {
                const isSelected = selectedGalleryImages.includes(img.publicUrl);
                const isAlreadySelected = initialSelectedImages.includes(img.publicUrl);
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (singleImageMode) {
                        ...
                        if (isSelected) {
                          setSelectedGalleryImages(prev => prev.filter(url => url !== img.publicUrl));
                        } else {
                          setSelectedGalleryImages(prev => [...prev, img.publicUrl]);
                        }
                      }
                    }}
                  ...
                  >
                    ...
                    {isAlreadySelected && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50">
                        <span className="text-white text-xs font-medium">Already Selected</span>
                      </div>
                    )}
                  </button>
                );
          ...
          {(selectedFiles.length > 0 || selectedGalleryImages.length > 0) && (
            ...
            >
              {isUploading ? (
                ...
                <>
                  Select {selectedGalleryImages.length || selectedFiles.length} {(selectedGalleryImages.length || selectedFiles.length) === 1 ? 'Image' : 'Images'}
                </>
              )}
            </Button>
          )}
